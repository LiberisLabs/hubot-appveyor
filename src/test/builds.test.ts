...
test('finbot > lists builds', (t) => {
  ...
  const buildResponse = {
    ...
    body: {
      ...
      accountName: account,
      builds: [
        { version: '1.2.3', status: 'success', link: 'http://link/1', colour: '#7CD197' },
        { version: '2.2.3', status: 'success', link: 'http://link/2', colour: '#7CD197' },
        { version: '3.2.3', status: 'failed', link: 'http://link/3', colour: '#D17C8C' },
        { version: '4.2.3', status: 'wot', link: 'http://link/4', colour: '#CCCCCC' },
        { version: '5.2.3', status: 'success', link: 'http://link/5', colour: '#7CD197' },
      ]
    }
  };
  ...
  for (let i = 0; i < 5; i++) {
    ...
    t.is(attachment.text, buildResponse.body.builds[i].status);
