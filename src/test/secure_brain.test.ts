...
test('secure_brain > sets object values', (t) => {
  ...
  const secureBrain = new SecureBrain(robotBrain, 'key');

  let returnValue = secureBrain.set('some key', { some: 'value' });
  assert.strictEqual(returnValue, secureBrain);
  sinon.assert.calledWith(brainSetSpy, 'some key', { secure: '621b48edf4ddc327eb7e721b5d266f982d392465dd5c5b2574887a61ac01581f' });

  brainSetSpy.reset();

  returnValue = secureBrain.set('some key', { some: { other: 'value' } });
  assert.strictEqual(returnValue, secureBrain);
  sinon.assert.calledWith(brainSetSpy, 'some key', { secure: '59ed1e0f05a7a27a84c4016596647f4e4e4e57ac30bdf43d09e462ebc4a6ce7d' });
});

test('secure_brain > gets object values', (t) => {
  const robotBrain = new MockRobotBrain();
  const brainGetStub = sinon.stub(robotBrain, 'get').returns({ secure: '621b48edf4ddc327eb7e721b5d266f982d392465dd5c5b2574887a61ac01581f' });
  ...
  brainGetStub.reset();
  brainGetStub.returns({ secure: '59ed1e0f05a7a27a84c4016596647f4e4e4e57ac30bdf43d09e462ebc4a6ce7d' });
...
test('secure_brain > sets a string value', (t) => {
  ...
  const secureBrain = new SecureBrain(robotBrain, 'secret');

  const returnValue = secureBrain.set('my key', 'this is a string');


  sinon.assert.calledWith(brainSetSpy, 'my key', { secure: 'b4216c9c0b30be78126100989f6a5f15850fe11e00b1951b07d5422cf017aa78' });
  assert.strictEqual(returnValue, secureBrain);
});

test('secure_brain > gets a string value', (t) => {
  const robotBrain = new MockRobotBrain();
  const brainGetStub = sinon.stub(robotBrain, 'get').returns({ secure: 'b4216c9c0b30be78126100989f6a5f15850fe11e00b1951b07d5422cf017aa78' });
...
test('secure_brain > gets a malformed value', (t) => {
  const robotBrain = new MockRobotBrain();
  const brainGetStub = sinon.stub(robotBrain, 'get').returns({ secure: 'b4216c9c0b30be78126100989f6a5f15850fe11e00b1951b07d5422cf017aa79' });
